...
- **Inserimento PC da parte degli utenti**: pagina `/aggiungi` pubblica; i prodotti inviati
  entrano in **coda di moderazione** e diventano visibili solo dopo l'approvazione admin
  (badge "Da approvare" in dashboard). Anti-abuso: captcha, honeypot, rate limit (3/ora per IP)
...
- **Grafica ufficiale rizzoaiacademy.com**: stessa palette (sfondo `#0e0c17`, card `#0d101b`,
  ...
  Header e footer linkano il sito dell'Academy
...
| `ADMIN_TOKEN`   | —                          | Token admin (dashboard `/admin` + API)   |
...
| POST   | `/api/hardware`                      | Inserimento pubblico PC → coda moderazione ⚠️   |
...
⚠️ = pubblico ma protetto da captcha, honeypot e rate limit (3 inserimenti/ora per IP);
richiede poi approvazione admin prima di essere visibile.
...
- Genera ADMIN_TOKEN con `openssl rand` scrivendolo DIRETTAMENTE nel file,
...
3. Crea il segreto SENZA leggerlo né stamparlo (serve solo l'ADMIN_TOKEN: il sito è
   pubblico, l'autenticazione resta solo per la dashboard `/admin`):
     cp .env.example .env
     sed -i "s|^ADMIN_TOKEN=.*|ADMIN_TOKEN=$(openssl rand -hex 32)|" .env
     chmod 600 .env
   Verifica SOLO la struttura: `grep -c '^ADMIN_TOKEN=.' .env` (atteso: 1).
4. Consegna il segreto al proprietario in modo sicuro: salvalo in un file FUORI dalla repo
...
7. VERIFICHE (riporta solo questi esiti, mai segreti):
   - `curl -s -o /dev/null -w "%{http_code}" https://<DOMINIO>/` → 200 (catalogo pubblico)
   - `curl -s -o /dev/null -w "%{http_code}" https://<DOMINIO>/aggiungi` → 200 (form inserimento)
   ...
   - test umano: Simone apre l'URL e vede il catalogo con ≥ 13 prodotti, poi prova
     il form "+ Aggiungi un PC" (il prodotto di test va poi eliminato dalla dashboard admin)
...
- Il **token admin** arriva a Carlos tramite il file sicuro (punto 4 del brief): Simone lo
  salva nel password manager, lo condivide con Carlos (es. Bitwarden Send), poi
  **cancella il file**.
- Carlos usa il token su `https://<dominio>/admin` per la dashboard (approvare i PC inviati
  dagli utenti, moderare commenti), oppure lo mette come `HARDWARE_HUB_ADMIN_TOKEN` nel suo
  agente AI (vedi sezione "Gestione da agenti AI").
- Periodicamente: controllare la dashboard per i prodotti "⏳ Da approvare".

---

## 🔒 Visibilità del sito

Il sito è **pubblico** (niente login né chiavi per gli utenti): chiunque ha il link può
vedere il catalogo, commentare e proporre nuovi PC. Resta però volutamente **a basso
profilo**:

1. **Anti-indicizzazione**: `robots.txt` con `Disallow: /`, header `X-Robots-Tag: noindex,
   nofollow, noarchive, nosnippet` su ogni risposta, meta robots noindex su tutte le pagine.
   Per renderlo indicizzabile in futuro basta rimuovere queste direttive.
2. **Anti-abuso attivo**: captcha self-hosted, honeypot, rate limit per IP, coda di
   approvazione per i nuovi prodotti, trappola anti-bot sui path-esca.
3. **L'unica area protetta è `/admin`**: token segreto (`ADMIN_TOKEN`), login rate-limited.
...
> ⚠️ **Nota**: i certificati HTTPS finiscono nei log pubblici di Certificate Transparency
> (crt.sh), quindi un sottodominio HTTPS è *tecnicamente* enumerabile. Visto che il sito è
> pubblico, non è un problema: le difese reali sono l'anti-abuso e la moderazione admin.
> Se si vuole ridurre la visibilità del sottodominio si può usare un certificato wildcard
> (`*.esempio.it`, richiede validazione DNS — Caddy lo gestisce con il plugin DNS del provider).
