...
// Tronca testi lunghi per non uscire dal placeholder SVG.
const clip = (s, n) => {
  const t = String(s || '');
...
export function placeholderSvg(product) {
  ...
  const name = clip(product.name, 38);
  const sub = clip(`${product.brand} · ${product.category}`, 48);
  ...
  <text x="480" y="430" text-anchor="middle" font-family="Segoe UI, system-ui, sans-serif" font-size="40" font-weight="700" fill="#e6eaf2">${esc(name)}</text>
  <text x="480" y="475" text-anchor="middle" font-family="Segoe UI, system-ui, sans-serif" font-size="22" fill="#9aa5b8">${esc(sub)}</text>
</svg>`;
}
